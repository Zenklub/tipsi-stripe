...
public class Converters {
  ...
  public static WritableMap convertPaymentIntentToWritableMap(PaymentIntent intent) {
    WritableMap map = Arguments.createMap();
    WritableMap latestPaymentErrorMap = Arguments.createMap();

    PaymentIntent.Error latestPaymentError = intent.getLastPaymentError();

    if (latestPaymentError != null) {
      latestPaymentErrorMap.putString("code", latestPaymentError.code);
      latestPaymentErrorMap.putString("message", latestPaymentError.message);
      latestPaymentErrorMap.putString("type", latestPaymentError.type.toString());
      map.putMap("latestPaymentError", latestPaymentErrorMap);
    } else {
      map.putNull("latestPaymentError");
    }

    if (intent.getCancellationReason() != null) {
      map.putString("cancellationReason", intent.getCancellationReason().toString());
    } else {
      map.putNull("cancellationReason");
    }

    if (intent.getNextActionType() != null) {
      map.putString("nextActionType", intent.getNextActionType().toString());
    } else {
      map.putNull("nextActionType");
    }

    map.putInt("amount", Math.toIntExact(intent.getAmount()));
    map.putString("currency", intent.getCurrency());
    map.putString("confirmationMethod", intent.getConfirmationMethod());
    map.putString("description", intent.getDescription());
    map.putString("id", intent.getId());
    map.putString("paymentMethodId", intent.getPaymentMethodId());
    map.putString("receiptEmail", intent.getReceiptEmail());
    map.putInt("canceledAt", Math.toIntExact(intent.getCanceledAt()));
    map.putInt("createdAt", Math.toIntExact(intent.getCreated()));
    map.putString("status", intent.getStatus().toString());
