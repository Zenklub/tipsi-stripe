...
RCT_EXPORT_METHOD(retrievePaymentIntent:(NSString *)clientSecret
    ...
    [api retrievePaymentIntentWithClientSecret:clientSecret completion:^(STPPaymentIntent * _Nullable intent, NSError * _Nullable error) {
        ...
        [self resolvePromise: [self convertPaymentIntent: intent]];
    }];
}
...
- (NSString*) convertSTPPaymentIntentConfirmationMethodToString:(STPPaymentIntentConfirmationMethod) confirmationMethod {
    NSString *result = nil;

    switch(confirmationMethod) {
        case STPPaymentIntentConfirmationMethodAutomatic:
            result = @"automatic";
            break;
        case STPPaymentIntentConfirmationMethodManual:
            result = @"manual";
            break;
        case STPPaymentIntentConfirmationMethodUnknown:
            result = @"unknown";
            break;
...
- (NSDictionary<TPSStripeType(PaymentIntent), id>*)convertPaymentIntent:(STPPaymentIntent*)intent {
    if (!intent) {
    ...
    [result setValue:intent.canceledAt forKey:TPSStripeParam(PaymentIntent, canceledAt)];
    [result setValue:[NSNumber numberWithInt: (int) intent.amount]forKey:TPSStripeParam(PaymentIntent, amount)];
    [result setValue:intent.currency forKey:TPSStripeParam(PaymentIntent, currency)];
    [result setValue:intent.description forKey:TPSStripeParam(PaymentIntent, description)];
    [result setValue:intent.receiptEmail forKey:TPSStripeParam(PaymentIntent, receiptEmail)];
    [result setValue:[self convertSTPPaymentIntentConfirmationMethodToString: intent.confirmationMethod] forKey:TPSStripeParam(PaymentIntent, confirmationMethod)];
    [result setValue:intent.created forKey:TPSStripeParam(PaymentIntent, createdAt)];
    [result setValue:intent.canceledAt forKey:TPSStripeParam(PaymentIntent, canceledAt)];
    [result setValue:[self convertSTPPaymentIntentNextActionTypeToString: intent.nextAction.type] forKey:TPSStripeParam(PaymentIntent, nextActionType)];
    [result setValue:intent.paymentMethodId forKey:TPSStripeParam(PaymentIntent, paymentMethodId)];
